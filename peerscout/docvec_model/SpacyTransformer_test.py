...
class TestSpacyTransformer(object):
  ...
  def test_should_extract_using_pipe(self):
    assert (
      strip_all(SpacyTransformer(use_pipe=True, use_progress=False).transform(['manuscript'])) ==
      ['manuscript|NOUN']
    )

  def test_should_extract_without_pipe(self):
    assert (
      strip_all(SpacyTransformer(use_pipe=False, use_progress=False).transform(['manuscript'])) ==
      ['manuscript|NOUN']
    )

  def test_should_extract_using_progress(self):
    assert (
      strip_all(SpacyTransformer(use_progress=True).transform(['manuscript'])) ==
      ['manuscript|NOUN']
    )
